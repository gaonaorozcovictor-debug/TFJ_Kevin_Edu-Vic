...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  :root {
    ...
    --ok: #1a6b45; --range: #2cb67d; --warn: #e35f1f; --bad: #e05252;
    --radius: 12px;
  }
  body { font-family: 'DM Sans', sans-serif; background: var(--light); color: var(--dark); min-height: 100vh; }

  .navbar { background: var(--dark); padding: 0 32px; height: 60px; display: flex; align-items: center; justify-content: space-between; position: sticky; top: 0; z-index: 100; }
  ...
  .nav-badge { font-size: .8rem; color: #aaa; padding: 0 12px; }
  ...
  .page-title small { display: block; font-family: 'DM Sans', sans-serif; font-size: .85rem; font-weight: 400; color: var(--muted); margin-top: 4px; }

  .alert { padding: 12px 16px; border-radius: var(--radius); font-size: .875rem; margin-bottom: 20px; border-left: 4px solid; }
  ...
  .card-header { padding: 20px 24px 16px; border-bottom: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center; }
  .card-title { font-weight: 600; font-size: 1rem; }
  .card-body { padding: 20px 24px; }

  .upload-zone { border: 2px dashed var(--border); border-radius: var(--radius); padding: 28px; text-align: center; transition: border-color .2s, background .2s; }
  ...
  input[type="file"] { display: block; margin: 0 auto 12px; font-family: inherit; font-size: .85rem; }
  .btn-upload { background: var(--accent); color: #fff; border: none; border-radius: 8px; padding: 9px 22px; cursor: pointer; font-family: inherit; font-size: .875rem; font-weight: 500; transition: background .2s; }
  .btn-upload:hover { background: #c9531a; }

  .table-wrap { overflow-x: auto; max-height: 360px; overflow-y: auto; }
  table { width: 100%; border-collapse: collapse; font-size: .875rem; }
  thead th { background: var(--light); padding: 10px 12px; text-align: left; font-weight: 600; font-size: .75rem; text-transform: uppercase; letter-spacing: .05em; color: var(--muted); position: sticky; top: 0; }
  ...
  .modal { background: var(--white); border-radius: var(--radius); padding: 32px; max-width: 420px; width: 90%; box-shadow: 0 20px 60px rgba(0,0,0,.2); }
  .modal h3 { font-size: 1.1rem; font-weight: 600; margin-bottom: 10px; }
  .modal p { color: var(--muted); font-size: .875rem; line-height: 1.6; margin-bottom: 24px; }
